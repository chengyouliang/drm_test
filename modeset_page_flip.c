...
static int modeset_create_fb(int fd, struct buffer_object *bo, uint32_t color)
{
    ...
    drmModeAddFB(fd,bo->width,bo->height,24,32,bo->pitch,bo->handle,&bo->fd_id);
...
static void modeset_page_flip_handler(int fd, uint32_t frme,uint32_t sec,uint32_t usec,void *data)
{
    static int i = 0;
    uint32_t crtc_id = *(uint32_t *)data;
    i ^= 1;
    drmModePageFlip(fd,crtc_id,buf[i].fd_id,DRM_MODE_PAGE_FLIP_EVENT,data);
    usleep(500000); 
}

static void  sigint_hander(int arg)
{
    terminate = 1;
}
int main(int argc, char **argv[])
{
    ...
    drmModeSetCrtc(fd,crtc_id,buf[0].fd_id,0,0,&conn_id,1,&conn->modes[0]);
    drmModePageFlip(fd, crtc_id, buf[0].fd_id,
